...
| `cli.py` | `physgate decompose` (the configuration from explicit inputs, the one call, the run started) and `physgate queue list/resolve`. `physgate run` and `resume` arrive with the dispatcher |
