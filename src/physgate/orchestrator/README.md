...
| `managed.py` | The managed-settings tier, which `--setting-sources ""` does not govern and the agent does not write: the system managed paths are recorded, never touched; after every invocation a non-empty remote-settings cache or a changed system path is drift, and drift halts the run. Detection, not prevention: `CLAUDE_CODE_REMOTE_SETTINGS_PATH` was measured not to replace the remote fetch, so it is not used |
