"""ROBUSTNESS DIAGNOSTIC (post-hoc, does not change any verdict):
the same selection rule with every fit replicated over three tmu seeds and the
median taken, to test whether the selection survives sweep estimation noise.
...
def main(outpath):
    ...
    for c, T, s in itertools.product(CLAUSES, T_VALUES, S_VALUES):
        accs, eces, has, hbs = [], [], [], []
        for rep in (1000, 1001, 1002):
            tm = E.fit_tm(Xfit, yfit, c, T, s, EPOCHS, seed=rep)
            pred, _, p_val = E.tm_predict(tm, Xval, T)
            accs.append(float((pred == yval).mean())); eces.append(E.ece(p_val, yval))
            tb = E.fit_tm(XB, yB, c, T, s, EPOCHS, seed=rep)
            hbs.append(conf(E.tm_predict(tb, val_b_id, T)[2])
                       - conf(E.tm_predict(tb, val_b_ood, T)[2]))
            ta = E.fit_tm(XA, yA, c, T, s, EPOCHS, seed=rep)
            has.append(conf(E.tm_predict(ta, val_a_id, T)[2])
                       - conf(E.tm_predict(ta, val_a_ood, T)[2]))
        acc, ece = float(np.median(accs)), float(np.median(eces))
        ha, hb = float(np.median(has)), float(np.median(hbs))
