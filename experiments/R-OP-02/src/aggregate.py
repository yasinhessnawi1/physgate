...
def main() -> None:
    ...
    summary["ratios_over_baseline"] = {
        arm: {
            "c2_diff_p95": round(
                summary["impls"][arm]["c2_diff_p95_ms"]["mean"] / base["c2_diff_p95_ms"]["mean"], 2
            ),
            "c3_traverse_p95": round(
                summary["impls"][arm]["c3_traverse_p95_ms"]["mean"]
                / base["c3_traverse_p95_ms"]["mean"], 2
            ),
            "write_p50": round(
                summary["impls"][arm]["write_p50_ms"]["mean"] / base["write_p50_ms"]["mean"], 2
            ),
        }
        for arm in ("openpersona-st", "openpersona-hash")
    }
    summary["c5"] = json.loads((METRICS / "c5_loc.json").read_text())
    (METRICS / "summary.json").write_text(json.dumps(summary, indent=2))
    print(json.dumps(summary["ratios_over_baseline"], indent=2))
