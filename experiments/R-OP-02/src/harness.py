...
def expected_closure(nodes: dict[str, dict], start: str) -> set[str]:
...
def run(store: Any, work: generator.Workload) -> dict:  # noqa: ANN401 - Protocol duck type
    """Replay ``work`` against ``store``, scoring C1, C2, C3 and write latency."""
    c1 = {
        "cross_role_attempted": 0, "cross_role_rejected": 0,
        "interface_attempted": 0, "interface_rejected": 0,
        "missing_unit_attempted": 0, "missing_unit_rejected": 0,
        "legal_attempted": 0, "legal_accepted": 0,
        "creates_attempted": 0, "creates_accepted": 0,
        "rollbacks_attempted": 0, "rollbacks_completed": 0,
    ...
    node_payloads: dict[str, dict[int, dict]] = {}
    ...
    for op in work.ops:
        if op.kind == "create":
            ...
            node_payloads.setdefault(op.node_id, {})[result.revision] = op.payload
            last_payload[op.node_id] = op.payload
        ...
        # ---- a write operation -------------------------------------------
        write_index += 1
        bucket = {"cross_role": "cross_role", "missing_unit": "missing_unit",
                  "interface": "interface"}.get(op.fault, "legal")
        ...
        if op.expect == "accept":
            ...
            if result.accepted:
                ...
                if result.reason != wanted_reason:
                    ...
                    failures.append(
                        f"{bucket} rejected for the wrong reason: {result.reason}"
                    )
    ...
    return {
        ...
        "c1": c1,
        "c2_diff_ms": {
            "p50": percentile(diff_ms, 50), "p95": percentile(diff_ms, 95),
            "n": len(diff_ms),
        },
        "c3_traverse_ms": {
            "p50": percentile(traverse_ms, 50), "p95": percentile(traverse_ms, 95),
            "n": len(traverse_ms),
        },
        "write_ms": {
            "p50": percentile(write_ms, 50), "p95": percentile(write_ms, 95),
            "n": len(write_ms),
        },
        "rollback_ms": {
            "p50": percentile(rollback_ms, 50), "p95": percentile(rollback_ms, 95),
            "n": len(rollback_ms),
        },
