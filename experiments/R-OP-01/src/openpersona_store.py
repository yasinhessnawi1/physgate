...
Uses the real ``persona-core`` package: ``SelfFactsStore`` over ``ChromaBackend``
with the project embedder and the package's JSONL audit logger. Nothing is
...
class OpenPersonaStore:
    ...
    def recover(self) -> None:
        """Rebuild the revision index from the durable store."""
        # ADAPTER:revision
    ...
    def _head_chunk(self, node_id: str) -> PersonaChunk | None:
        ...
        for chunk in found:
            if chunk.provenance is not None and chunk.provenance.superseded_by is None:
                return chunk
        return None
    ...
    def _head_chunks(self, node_ids: list[str]) -> dict[str, PersonaChunk]:
        ...
        for chunk in found:
            ...
            if prov is not None and prov.superseded_by is None:
