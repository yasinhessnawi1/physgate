...
class Workload:
    ...
    legal_payloads: dict[str, list[dict]] = field(default_factory=dict)


def _quantities(rng: random.Random, n: int, writer: str, *, with_units: bool = True) -> dict:
    out: dict = {}
...
def _closure_size(nodes: dict[str, dict], start: str) -> int:
    seen: set[str] = set()
...
def build(seed: int) -> Workload:
    ...
    # ---- create ops ------------------------------------------------------
    ops: list[Op] = []
    legal_payloads: dict[str, list[dict]] = {nid: [] for nid in all_ids}
    idx = 0
    for nid in interface_ids + member_ids:  # interfaces first: decomposition
        payload = copy.deepcopy(nodes[nid])
        ops.append(
            Op(index=idx, kind="create", node_id=nid, actor_role=nodes[nid]["owner_role"],
               payload=payload, expect="accept")
        )
    ...
    def _mutated(nid: str, writer: str, *, with_units: bool) -> dict:
    ...
    legal_targets += [rng.choice(member_ids) for _ in range(N_WRITES - N_CROSS_ROLE
                                                           - N_MISSING_UNIT - N_INTERFACE_WRITES
                                                           - N_HIGH_DEGREE_WRITES)]
    for nid in legal_targets:
        owner = nodes[nid]["owner_role"]
        writes.append(Op(index=-1, kind="write", node_id=nid, actor_role=owner,
                         payload=_mutated(nid, owner, with_units=True), expect="accept"))

    for _ in range(N_CROSS_ROLE):
        nid = rng.choice(member_ids)
        owner = nodes[nid]["owner_role"]
        intruder = rng.choice([r for r in ALL_ROLES if r != owner])
        writes.append(Op(index=-1, kind="write", node_id=nid, actor_role=intruder,
                         payload=_mutated(nid, intruder, with_units=True),
                         expect="reject:cross_role_write", fault="cross_role"))
    ...
    for _ in range(N_MISSING_UNIT):
        ...
        writes.append(Op(index=-1, kind="write", node_id=nid, actor_role=owner,
                         ...
                         expect="reject:quantity_missing_unit", fault="missing_unit"))

    for _ in range(N_INTERFACE_WRITES):
        nid = rng.choice(interface_ids)
        writes.append(Op(index=-1, kind="write", node_id=nid, actor_role=INTERFACE_OWNER,
                         payload=_mutated(nid, INTERFACE_OWNER, with_units=True),
                         expect="reject:interface_immutable", fault="interface"))
    ...
    for op in merged:
        ops.append(
            Op(index=idx, kind=op.kind, node_id=op.node_id, actor_role=op.actor_role,
               payload=op.payload, expect=op.expect, to_ordinal=op.to_ordinal, fault=op.fault)
        )
    ...
    return Workload(seed=seed, nodes=nodes, ops=ops, deepest_node_id=deepest,
                    counts=counts, legal_payloads=legal_payloads)
